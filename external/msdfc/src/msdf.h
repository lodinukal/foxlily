...
static inline uint32_t ex_utf8(const char *c) {
  ...
  return val;
   ...
   #ifndef MSDF_malloc
   #include <stdlib.h>
   #define MSDF_malloc(x,u)  ((void)(u),malloc(x))
   #define MSDF_free(x,u)    ((void)(u),free(x))
   #endif
...
int ex_msdf_glyph_mem(stbtt_fontinfo *font, uint32_t c, size_t w, size_t h, float *bitmap, ex_metrics_t *metrics, int autofit)
{
  ...
  indices_t *contours = MSDF_malloc(sizeof(indices_t) * contour_count);
  int j = 0;
  for (int i=0; i<=num_verts; i++) {
    if (verts[i].type == STBTT_vmove) {
      if (i > 0) {
  ...
  contour_t *contour_data = MSDF_malloc(sizeof(contour_t) * contour_count);
  double cscale = 64.0;
  for (int i=0; i<contour_count; i++) {
    size_t count = contours[i].end - contours[i].start;
    contour_data[i].edges = MSDF_malloc(sizeof(edge_segment_t) * count);
  ...
  int *corners = MSDF_malloc(sizeof(int) * corner_count);
  int corner_index = 0;
  for (int i=0; i<contour_count; ++i) {
    ...
    if (corner_index == 0) {
      ...
      if (contour_data[i].edge_count >= 3) {
        ...
        MSDF_free(contour_data[i].edges);
        contour_data[i].edges = MSDF_malloc(sizeof(edge_segment_t) * 7);
        contour_data[i].edge_count = 0;
        int index = 0;
  ...
  for (int i=0; i<contour_count; i++) {
    if (contour_data[i].edge_count == 1) {
      ...
      MSDF_free(contour_data[i].edges);
      contour_data[i].edges = MSDF_malloc(sizeof(edge_segment_t) * 3);
  ...
  // calculate windings
  int *windings = MSDF_malloc(sizeof(int) * contour_count);
  for (int i=0; i<contour_count; i++) {
  ...
  contour_sd = MSDF_malloc(sizeof(multi_distance_t) * contour_count);
  ...
  for (int i=0; i<contour_count; i++)
    MSDF_free(contour_data[i].edges);
  MSDF_free(contour_data);
  MSDF_free(contour_sd);
  MSDF_free(contours);
  MSDF_free(windings);
  MSDF_free(verts);
  ...
  clashes_t *clashes = MSDF_malloc(sizeof(clashes_t) * w * h);
  size_t cindex = 0;
...
float *ex_msdf_glyph(stbtt_fontinfo *font, uint32_t c, size_t w, size_t h, ex_metrics_t *metrics, int autofit)
{
  float *bitmap = MSDF_malloc(sizeof(float)*3*w*h);
